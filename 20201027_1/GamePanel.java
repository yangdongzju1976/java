...
//	int foodX;
	...
	public GamePanel(){// 类的构造器 。生成对象的时候自动运行。
		...
		timer = new Timer(100,new ActionListener(){
			@Override
				...
				//吃食物。吃完消失。（1）
				//吃完食物，蛇身子变长。  
				if (snakeX[0]==food1.x&&snakeY[0]==food1.y||snakeX[0]==food2.x&&snakeY[0]==food2.y)
				{
					length++;
				...
				//食物的坐标随机发生改变。Random();
				Random r=new Random();
				if (snakeX[0]==food1.x&&snakeY[0]==food1.y)
				{
					//length++;
					food1.x=r.nextInt(30)*25+25;    //foodX的坐标在[25,750]之间25的倍数 r.nextInt(30)产生[0,30)之间的整数
					//System.out.println(food1.x+"\n");
					food1.y=r.nextInt(27)*25+75;//foodY的坐标在[75,725]之间25的倍数。r.nextInt(27)产生[0,27)之间的整数
					//System.out.println(food1.y+"\n");
					scores++;
				}
				if (snakeX[0]==food2.x&&snakeY[0]==food2.y)
				{
					//length++;
					food2.x=r.nextInt(30)*25+25;    //foodX的坐标在[25,750]之间25的倍数 r.nextInt(30)产生[0,30)之间的整数
					//System.out.println(food1.x+"\n");
					food2.y=r.nextInt(27)*25+75;//foodY的坐标在[75,725]之间25的倍数。r.nextInt(27)产生[0,27)之间的整数
					//System.out.println(food1.y+"\n");
					scores+=3;
				}
	...
	@Override 
		...
		*/
			...
			g.setColor(Color.BLUE);
				//g.setColor(new Color(255,100,200));
			g.fillRect(food1.x,food1.y,25,25);
			g.setColor(Color.YELLOW);
			g.fillRect(food2.x,food2.y,25,25);
			...
			if (isDie ){//游戏暂停
				...
				g.setColor(new Color(220,0,0));//设置颜色
				g.setFont(new Font("宋体",Font.BOLD,40));//设置字体
				g.drawString("小蛇死亡，点击空格重新开始游戏",80,330);//画上文字。坐标（250，330）
			}
			...
			//得分
				...
				g.drawString("得分："+scores,685,65);//画上文字。坐标（250，330）
